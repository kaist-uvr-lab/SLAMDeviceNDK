                               ...
                               const cv::Mat& K,int flags)
{
    ...
    // Pose estimation
    bool correspondence = cv::solvePnP( list_points3d, list_points2d, K, distCoeffs, rvec, tvec,
...
void PnPProblem::estimatePoseRANSAC( const std::vector<cv::Point3f> &list_points3d, // list with model 3D coordinates
                                     ...
                                     const cv::Mat& K, cv::Mat& rvec, cv::Mat& tvec, int flags, cv::Mat &inliers, int iterationsCount,  // PnP method; inliers container
...
//std::vector<cv::Point2f> PnPProblem::verify_points(Mesh *mesh, const cv::Mat& K)
//{
//    std::vector<cv::Point2f> verified_points_2d;
//    for( int i = 0; i < mesh->getNumVertices(); i++)
//    {
//        cv::Point3f point3d = mesh->getVertex(i);
//        cv::Point2f point2d = this->backproject3DPoint(point3d,K);
...
cv::Point2f PnPProblem::backproject3DPoint(const cv::Point3f &point3d, const cv::Mat& K)
{
    //std::cout << K.type() << " " << P_matrix_.type() << std::endl;
    ...
    point2d_vec = K * P_matrix_ * point3d_vec;
...
//bool PnPProblem::backproject2DPoint(const Mesh *mesh, const cv::Mat& K, const cv::Point2f &point2d, cv::Point3f &point3d)
//{
...
//    cv::Mat X_c = K.inv() * point2d_vec ; // 3x1
//
